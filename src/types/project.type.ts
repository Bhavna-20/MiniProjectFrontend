...
export type UnScheduledProjectEntity = {
  clientId: string;
  name: string;
...
export type UnScheduledProjectPayload = {
  clientId: string;
  unScheduledProjectId: string;
};
...
export const ProjectCreatePayloadSchema = object({
  ...
  stateId: string()
    ...
    .when('clientId', (clientId, schema) => {
      if (clientId) {
        return schema.min(1).required().label('State');
      }
    })
    .label('State'),
...
export const ScheduleProjectSchema = object({
  ...
  billingRate: number().optional().label('Billing Amount'),
});
